...
def title(msg):
    ...
    chromedriver_dir = r"C:\chromedriver_win32\chromedriver.exe"
...
async def on_ready():
    ...
    await bot.change_presence(status=discord.Status.online, activity=discord.Game("응애"))
...
async def 재생(ctx, *, msg):
    if not vc.is_playing():
        ...
        await ctx.send(" result + 을(를) 대기열로 추가시켰어용!")
